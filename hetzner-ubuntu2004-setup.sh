...
: <<'end_header_info'
Original script (c) Andrey Prokopenko job@terem.fr
Modified by e12e for 20.04 support.
fully automatic script to install Ubuntu 20.04 LTS with ZFS root on Hetzner VPS
...
echo "======= create zfs pools and datasets =========="
  ...
  if [[ ${#v_selected_disks[@]} -gt 3 ]]; then
    pools_mirror_option=raidz2
  elif [[ ${#v_selected_disks[@]} -gt 1 ]]; then
